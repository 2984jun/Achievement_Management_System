    ...
    public partial class View_College : Form
    {
        ...
        private void showinf() 
        {
            using (SqlConnection con = new SqlConnection(strConn))
            {
                ...
                try
                {
                    string sql = "SELECT College_id AS 学院编号,Sname AS 学院名称,major_amount AS 专业数量,totle_people AS 学院总人数,Dean AS 院长 FROM School ORDER BY College_id";
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            using(SqlConnection con=new SqlConnection(strConn)) 
            {
                ...
                try 
                {
                    if (this.dgv_College.CurrentCell != null)
                    {
                        string sql = "SELECT * FROM School s INNER JOIN College c ON s.college_id=c.college_id WHERE s.college_id='" +
