...
{
    public partial class MajorAddUpdate : Form
    {
                ...
                else
                {
                    using (SqlConnection con = new SqlConnection(strConn))
                    {
                        ...
                        try
                        {
                            ...
                            {
                                MessageBox.Show("专业编号或专业名称重复，请重新输入！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            }                           
                            else
                            {
                                string sql = "SELECT * FROM College WHERE college_id='" + this.txtCleId.Text.Trim() + "';";
                                cmd.CommandText = sql;
                                if (cmd.ExecuteScalar() == null)
                                {
                                    MessageBox.Show("不存在该学院，请重新输入学院编号！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                }
                                else 
                                {
                                    sql = "INSERT INTO Major(Major_id,college_id,Cname,class_num,totle_people,Gleader)VALUES('" + this.txtMajorId.Text.Trim() + "','" + this.txtCleId.Text.Trim() +
